...
- **Auto routing is live:** `modelMode` remains `manual` by default, with a per-chat Auto toggle. The first Auto message can choose any supported agent; follow-ups stay on the bound agent and may change only its model, with a cross-agent continuation suggestion when appropriate. _(Phase 7.)_
...
5. **Continuity:** the first message may route across agents; later messages stay on the chat's bound agent and may change its model. When another agent is a better fit, offer **Continue in <agent>** to start a new manual chat with the message prefilled; memory supplies cross-chat context.
